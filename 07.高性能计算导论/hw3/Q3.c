...
int main(void) {
    ...
    loc_bin_cts = (int*)malloc(bin_count * sizeof(int));
...
void Find_bins(
    ...
    */
    ...
    MPI_Reduce(loc_bin_cts, bin_counts,
        bin_count, MPI_FLOAT, MPI_SUM, 0, comm);
}  /* Find_bins */
...
void Get_input(int* bin_count_p, float* min_meas_p, float* max_meas_p,
    ...
    Check_for_error(local_ok, (char*)"Get_input",
        (char*)"data_count must be evenly divisible by comm_sz",
        comm);
