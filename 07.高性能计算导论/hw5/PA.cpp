...
#pragma omp parallel for num_threads(thread_cnt) schedule(dynamic)
    ...
    if (pid == 0) {
        ...
        printf("parallel finished in (include data distribution) %.4lf s,\nspeed up rate: %.2f\n",
            duration[0], ser_duration / duration[0]);
        printf("parallel finished in (enclude data distribution) %.4lf s,\nspeed up rate: %.2f\n",
            duration[1], ser_duration / duration[1]);

        double norm_beg = omp_get_wtime();
#pragma omp parallel for num_threads(thread_cnt)
        for (int i = 0; i < n; i++) {
            norm_diff += powl(ser_res[i] - res[i], 2.);
        }
        norm_diff = sqrtl(norm_diff);
        printf("the 2-norm computation finished in %4lf, error is %.10lf\n",
            omp_get_wtime() - norm_beg, norm_diff);
    }
    else {
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        printf("usage: %s [n] [thread count]\n", argv[0]);
        return -1;
    }
    int n = atoi(argv[1]), thread_cnt = atoi(argv[2]);
    run(n, thread_cnt);
    return 0;
}
