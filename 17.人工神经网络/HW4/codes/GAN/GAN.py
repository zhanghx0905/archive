...
class Generator(nn.Module):
    def __init__(self, num_channels, latent_dim, hidden_dim):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(latent_dim, 4 * hidden_dim, kernel_size=4),
        ...
        try:
            if os.path.exists(os.path.join(ckpt_dir, 'generator.bin')):
                ...
                path = os.path.join(ckpt_dir, str(max(int(name)
                                                      for name in os.listdir(ckpt_dir))), 'generator.bin')
    ...
    def restore(self, ckpt_dir):
        try:
            if os.path.exists(os.path.join(ckpt_dir, 'discriminator.bin')):
                ...
                path = os.path.join(ckpt_dir, str(max(int(name)
                                                      for name in os.listdir(ckpt_dir))), 'discriminator.bin')
        except BaseException:
            return None
        self.load_state_dict(torch.load(path))
        return os.path.split(path)[0]
