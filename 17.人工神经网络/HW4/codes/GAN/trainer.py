...
class Trainer(object):
    ...
    def train_step(self, real_imgs, fake_imgs, BCE_criterion):
        ...
        *   Returns:
        ...
        # TODO START
        D_real_logits = self._netD(real_imgs)
        loss_D_real = BCE_criterion(D_real_logits, torch.ones_like(D_real_logits))
        ...
        D_fake_logits = self._netD(fake_imgs.detach())
        loss_D_fake = BCE_criterion(D_fake_logits, torch.zeros_like(D_fake_logits))
        ...
        # record average D(fake_imgs)
        # TODO START
        D_fake_logits = self._netD(fake_imgs)
        loss_G = BCE_criterion(D_fake_logits, torch.ones_like(D_fake_logits))
        D_G_z2 = D_fake_logits.mean().item()
        loss_G.backward()
        # TODO END
    ...
    def train(self, num_training_updates, logging_steps, saving_steps):
        ...
        for i in tqdm(range(num_training_updates), desc='Training'):
            ...
            fake_imgs = self._netG(torch.randn(real_imgs.size(0), self._netG.latent_dim,
                                               1, 1).to(self._device))
