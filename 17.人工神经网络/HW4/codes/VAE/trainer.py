...
class Trainer(object):
    ...
    def loss_fn(self, target, recon, mu, log_var):
        ...
        *   Returns:
        ...
        '''
        # TODO START
        kl_loss = torch.mean(0.5 * torch.sum(mu**2 + torch.exp(log_var) - log_var - 1, -1))
        target, recon = target.view(-1, 1024), recon.view(-1, 1024)
    ...
    def train(self, num_training_updates, logging_steps, saving_steps):
        ...
        for i in tqdm(range(num_training_updates), desc='Training'):
            ...
            if (i + 1) % saving_steps == 0:
                ...
                for imgs, name in zip([dev_imgs, recons, samples], [
                                      'dev_imgs', 'reconstructions', 'samples']):
