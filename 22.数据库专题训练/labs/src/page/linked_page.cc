...
bool LinkedPage::PushBack(LinkedPage *pPage) {
  ...
  // LAB1 BEGIN
  // DONE: 链表结点后增加一个新的链表页面结点
  pPage->SetPrevID(_nPageID);
  pPage->SetNextID(_nNextID);
  auto oldNextID = _nNextID;
  // 调用 SetNextID 以设置 Modfied
  SetNextID(pPage->GetPageID());
  if (oldNextID != NULL_PAGE) {
    auto nextPage = make_unique<LinkedPage>(oldNextID);
    nextPage->SetPrevID(_nNextID);
  }
  // ALERT: ！！！一定要注意！！！
...
PageID LinkedPage::PopBack() {
  // LAB1 BEGIN
  // DONE: 删除下一个链表结点，返回删除结点的PageID
  PageID del = _nNextID;
  if (del == NULL_PAGE) {
    return NULL_PAGE;
  }
  auto toBeDeleted = make_unique<LinkedPage>(del);
  SetNextID(toBeDeleted->GetNextID());
  if (_nNextID != NULL_PAGE) {
    auto newNextPage = make_unique<LinkedPage>(_nNextID);
    newNextPage->SetPrevID(_nPageID);
  }
  toBeDeleted.reset();
  MiniOS::GetOS()->DeletePage(del);
  return del;
  // TIPS: 需要判断当前页面是否存在后续页面
  // TIPS:
